    ...
    if (cursor!!.moveToFirst()) {
        do {
            ...
            if(cursor.getInt(5)==1){
                 busStopOne=Select_db_BusStopInterface.BusStopRow(cursor.getLong(0),cursor.getInt(1).toString(),cursor.getLong(2),cursor.getLong(3),cursor.getInt(4),Select_db_BusStopInterface.Vehicle.BUS,cursor.getInt(6))
            }else{
                busStopOne=Select_db_BusStopInterface.BusStopRow(cursor.getLong(0),cursor.getInt(1).toString(),cursor.getLong(2),cursor.getLong(3),cursor.getInt(4),Select_db_BusStopInterface.Vehicle.TRAM,cursor.getInt(6))
            }
